...
export default function DaphnePortfolio() {
  ...
  const skillList = [
    { name: "AutoCAD", logo: "/logos/autocad.png", level: 90 },
    { name: "SketchUp", logo: "/logos/sketchup.png", level: 80 },
    { name: "Photoshop", logo: "/logos/photoshop.png", level: 70 },
    { name: "Illustrator", logo: "/logos/illustrator.png", level: 65 }
  ];
        ...
        <div className="flex justify-center gap-4 flex-wrap">
          {Object.keys(sections).map((key) => (
            <Button
              ...
              className={\`rounded-xl px-4 py-2 \${activeSection === key ? (darkMode ? "bg-pink-700 text-white" : "bg-[#FF8DA1] text-white") : "bg-white text-pink-700"}\`}
            >
              {sections[key].title}
            </Button>
          ))}
                    ...
                    <div key={skill.name} className="flex items-center gap-4">
                      ...
                      <div className="w-full">
                        ...
                        <div className={\`w-full h-3 rounded-full overflow-hidden \${darkMode ? "bg-gray-700" : "bg-pink-200"}\`}>
                          <div className={\`h-3 rounded-full \${darkMode ? "bg-pink-500" : "bg-[#FF8DA1]"}\`} style={{ width: \`\${skill.level}%\` }}></div>
                        </div>
                      </div>
                    </div>
                  ))}
